...
export default function PageArtist() {
  const [bands, setPost] = React.useState(null);
  let { _id } = useParams();

  React.useEffect(() => {
    axios.get("http://localhost:3001/bands/" + _id).then((response) => {
      setPost(response.data);
      console.log(response.data);
    });
  }, []);

  function addToFavourite(bands) {
    ...
    var config = {
      method: "patch",
      url: `http://localhost:3001/users/${userId}/favourite`,
      headers: {
  ...
  if (!bands) return "Cet artiste n'existe pas !";
    ...
    <div><div>
      ...
      <div class="ml-72 mt-20">
        <div class="text-red-700">
          <h1 class="font-semibold">{bands.name}</h1>
          <div class="coeur"
            onClick={() => addToFavourite(bands)}
          >
            Add to Favourite ❤️
          </div>
